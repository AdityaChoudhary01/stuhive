...
import { Trash2, Loader2, Edit, Globe, Lock } from "lucide-react";
...
export default function CollectionActions({ collection }) {
  ...
  const handleUpdate = async (e) => {
    ...
    const res = await updateCollection(
        collection._id, 
        { name, university, description, visibility }, 
        session?.user?.id
    );
    ...
    if (res.success) {
      ...
      setOpen(false);
      router.refresh(); // 🔥 Forces the server page to fetch the newly updated text & badges!
    } else {
      toast({ title: "Error", description: res.error, variant: "destructive" });
    }
    setIsUpdating(false);
        ...
        <DialogContent className="sm:max-w-md bg-[#0c0c10] border-white/10 text-white">
          ...
          <form onSubmit={handleUpdate} className="space-y-5 py-2">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-white/10">
                  ...
                  <Button type="submit" disabled={isUpdating || !name.trim()} className="bg-cyan-500 text-black font-bold hover:bg-cyan-400">
